...
<div class="columns">
    <div class="columns-row">
		...
		<?php
			if(isset($_GET['action']) == 'delete'){
				$deptId = $_GET['deptId'];
				$deptHead = $_GET['deptHead'];
				$cek = mysql_query("SELECT * FROM departments d,users u WHERE d.deptId = '$deptId' AND d.deptHead = u.Name;");
				$nfA = $cek['userId'];
    ...
    $query = "UPDATE users SET userType=2 WHERE Name LIKE ('".$deptHead."'); DELETE FROM departments WHERE deptId='$deptId';";
    // 
    $delete = mysqli_multi_query($dbhandle, $query);
					...
					if($delete){
						echo '<div class="alert alert-primary alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Data successfully removed! '.$nfA.'</div>';
				...
				if(mysql_num_rows($sql) == 0){
					...
					while($row = mysql_fetch_assoc($sql)){
						...
						<tr>
                            ...
                            <a href="department.php?deptId='.$row['deptId'].'">
						...
						echo '
							...
							<td>
								<a href="editdepartments.php?deptId='.$row['deptId'].'&deptHead='.$row['deptHead'].'" title="Edit Data" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
								<a href="viewdepartments.php?action=delete&deptId='.$row['deptId'].'&deptHead='.$row['deptHead'].'" title="Delete Data" onclick="return confirm(\'Are you sure to delete the data '.$row['deptName'].'?\')" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
							</td>
						</tr>
						';
						$no++;
